...
//									echo $sql_get_total_times;
                ...
                ?>
                                                    ...
                                                    <td class="<?php echo $td_class; ?>" id="<?php print ++$count_inside_fields ?>_<?php print $id_element ?>" valign="top">
                                                        <span class="sitetext1">
                <?php
                $sql_get_total_times = 'SELECT id FROM api_factchecks ';
                $sql_get_total_times .= ' WHERE DATE_FORMAT(insert_datetime, "%Y-%m-%d") = "' . $row['date_day'] . '" ';
                $sql_get_total_times .= ' AND user IS NOT NULL';
                $sql_get_total_times .= ' GROUP BY user ';
                $arr_uniq_per_day = $db_read->queryCol($sql_get_total_times);
                echo count($arr_uniq_per_day);
                ?>
                                                        </span>																	
                                                    </td>
                                                    <td class="<?php echo $td_class; ?>" id="<?php print ++$count_inside_fields ?>_<?php print $id_element ?>" valign="top">
...
//									echo $sql_get_total_times;
                                                    ...
                                                    <td class="<?php echo $td_class; ?>" id="<?php print ++$count_inside_fields ?>_<?php print $id_element ?>" valign="top">
                                                        <span class="sitetext1">
                                                            <?php
                                                            $sql_get_total_times = 'SELECT id FROM api_factchecks ';
                                                            $sql_get_total_times .= ' WHERE ' . $cond;
                                                            $sql_get_total_times .= ' AND user IS NOT NULL';
                                                            $sql_get_total_times .= ' GROUP BY user ';
                                                            $arr_uniq_per_day = $db_read->queryCol($sql_get_total_times);
                                                            echo count($arr_uniq_per_day);
...
//									echo $sql_get_total_times;
                                                    ...
                                                    <td class="<?php echo $td_class; ?>" id="<?php print ++$count_inside_fields ?>_<?php print $id_element ?>" valign="top">
                                                        <span class="sitetext1">
                                                            <?php
                                                            $sql_get_total_times = 'SELECT id FROM api_factchecks ';
                                                            $sql_get_total_times .= ' WHERE DATE_FORMAT(insert_datetime, "%Y-%m") = "' . $row['date_day'] . '" ';
                                                            $sql_get_total_times .= ' AND user IS NOT NULL';
                                                            $sql_get_total_times .= ' GROUP BY user ';
                                                            $arr_uniq_per_day = $db_read->queryCol($sql_get_total_times);
                                                            echo count($arr_uniq_per_day);
...
//			echo $sql_tops_total_api; 
        ...
        <?php echo $num_elements; ?>
                ...
                <?php echo $row_fact['post_date']; ?>
                                                        ...
                                                        <td class="<?php echo $td_class; ?>" id="<?php print ++$count_inside_fields ?>_<?php print $id_element ?>" > 
                                                            <?php
                                                            $sql_get_unique = 'SELECT id FROM api_factchecks';
                                                            $sql_get_unique .= " WHERE id_factcheck = '" . $row['id_factcheck'] . "' ";
                                                            $sql_get_unique .= " AND " . $cond;
                                                            $sql_get_unique .= ' AND user IS NOT NULL';
                                                            $sql_get_unique .= ' GROUP BY user ';
                                                            $arr_uniq_per_day = $db_read->queryCol($sql_get_unique);
                                                            echo count($arr_uniq_per_day);
                                                            ?>
                                                        </td>
                                                        <td class="<?php echo $td_class; ?>" id="<?php print ++$count_inside_fields ?>_<?php print $id_element ?>" > 
