				...
				<tr>
                    ...
                    <!--Start content-->
						...
						<table class="grid" width="100%" border="0" cellpadding="0" cellspacing="1">
						 ...
						 <?php
                        $sql_get_users = "SELECT a.link_identifier,a.id_link, a.link_identifier, a.link_content, a.snipped, a.insert_datetime, b.factcheck_link, b.post_title FROM factcheck_content2links as a LEFT JOIN factcheck_content as b ON a.id_factcheck = b.id_factcheck ";                      
                        ...
                        if ($_SESSION['file_on_server_search'] != "") {
                            ...
                            $sql_get_users .= " AND ( "
									...
									. "OR a.snipped  LIKE '%" . $db_write->escape($_SESSION['file_on_server_search']) . "%'  "
									. ")";
                        }
...
//                        echo $sql_get_users; 
				...
				<table width="100%" cellpadding="3" cellspacing="3" border="0" class="grid">
					<TR>
						...
						<th align="left" width="15%"> Snipped </th>
					...
					if (!empty($page_data) && is_array($page_data) && $page_data['totalItems'] > 0) {
						...
						<tr id="tr_link_<?php echo $id_element;?>" onmouseover="set_tr_color('<?php print $id_element ?>', '', 'in', '<?php print $tables_elements ?>');" onmouseout="set_tr_color('<?php print $id_element ?>', '<?php print $td_color ?>', 'out', '<?php print $tables_elements ?>')" >
							...
							<td class="<?php echo $td_class; ?>" id="<?php print ++$count_inside_fields ?>_<?php print $id_element ?>" valign="top">
								<span id="snipped_<?php echo $id_element;?>" class="txSnipped"><?php echo $row_factchecked['snipped'];?></span>
								<form action="javascript:;" id="frm_snipped_<?php echo $id_element;?>" style="display:none;" class="check2disable">
									<textarea id="tx_snipped_<?php echo $id_element;?>" class=""><?php // echo $row_factchecked['snipped'];?></textarea>
									<br/>
									<span id="msg_snipped_<?php echo $id_element;?>" class="check2disable"></span>
									
									<br/>
									<input type="submit" value="Modify snipped" class="input"  onclick="javascript:linksAction('modify_snipped', <?php echo $id_element;?>);" style="cursor:pointer">
								</form>
							</td>
...
function linksAction(action, id_link) {
	...
	switch(action) {
		case "modify_snipped":
			var snipped = $("#tx_snipped_"+id_link).val();
			snipped = snipped.trim();
			if(snipped == '') {				
				$('#msg_snipped_'+id_link).html('<font style="font-size: 15px;" color="red"><b>Empty snipped!</b></font>');
				$('#msg_snipped_'+id_link).show();
				return;
			}
			postData  = {action: action, id_link: id_link, snipped: snipped};
	...
	.done(function( msg ) {
		if(msg.length > 0) {
			...
			if(msg_type == 'snippedModified') {
				disableAllInputs();
				console.log(snipped_ret);
				$('#snipped_'+id_link).html(snipped_ret);
				$("#tx_snipped_"+id_link).val('');
				$('#msg_snipped_'+id_link).html('<font style="font-size: 15px;" color="red"><b>The snipped is modified!</b></font>');
				$('#snipped_'+id_link).show();
				$('#msg_snipped_'+id_link).show();
				$(".txSnipped").show();
				$("#tx_snipped_"+id_link).show();
			}
		}
	});
}
