...
if($id_factcheck ) {
		...
		if($action == 'add_link') { // add link
			...
			$res_check_link_exists = $db_read->queryOne($sql_check_link_exists);
			if (!empty($res_check_link_exists)) {
				//daca exista in db , dar este disable, il activam
				$id_link = $res_check_link_exists;
...
//					'unique_key' => $_POST['id_post'].md5($link_identifier), //Unique_key(id_post+md5_link_identifier)
				...
				if(!empty($_POST['snipped'])) {
					$table_fields_values['snipped'] = $db_write->escape(strip_tags($_POST['snipped']));
				}
...
//			$verify_link_exists =  "SELECT id_link FROM factcheck_content2links WHERE id_post = '" . $db_read->escape($_POST['id_post']) . "' AND id_factcheck = '" . $id_factcheck . "'   AND link_identifier = '$link_identifier'";
//			$res_check_link_exists = $db_read->queryOne($verify_link_exists);
...
//				if(!empty($res_check_link_exists) && $res_check_link_exists != $id_link) {
//					echo 'linkExists' . $separator . '0' . $separator . '';
//					exit();
//				} else {
//					$table_fields_values = array(
//						'link_content' => $db_write->escape($_POST['link']),
...
//					);
					if(!empty($_POST['snipped'])) {
						$table_fields_values['snipped'] = $db_write->escape($_POST['snipped']);
					}
